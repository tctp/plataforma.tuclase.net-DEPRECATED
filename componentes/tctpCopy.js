...
export default class extends React.Component {
  ...
  componentDidMount() {    
    ...
    this.setAncla(location.hash); //realiza la carga del hash en la url cuando el componente se cargó
  }
  ...
  createIconNode() {
    ...
    if (!comprobar) {
      nodes.forEach((node) => {        
        ...
        a.href = '#' + this.limpiarCadenaTexto(node.textContent); //limpia el titulo y da formato al titulo
        node.appendChild(span);        
        span.classList.add('iconCopy');
        span.setAttribute('id', a.hash.split('#')[1]); //asigna al atributo id
